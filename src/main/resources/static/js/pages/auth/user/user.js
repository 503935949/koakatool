...
$(function(){
    ...
    $remove.click(function () {
        var ids = $.map($('#userGrid').bootstrapTable('getSelections'), function (row) {
        ...
        if (confirm("你确定删除吗？")) {
            $.ajax({
                ...
                success: function (data, status) {
                    if (data.success) {
                        $('#userGrid').bootstrapTable('remove', {
                            field: 'job_number',
                            values: ids
                        });
                        successDialog('操作成功');
    ...
    $add.click(function () {
        ...
        var index = 0;//$('#userGrid').bootstrapTable('getData').length;
        $.ajax({
            type: "put",
            url: "/user/add",
            ...
            success: function (data, status) {
                ...
                if (data.success) {
                    $('#userGrid').bootstrapTable('insertRow', {
                        index: index,
                        row: {
                            job_number: $("#userIdInsert").val(),
                            name: $("#userNameInsert").val()
                        }
                    });
                    successDialog('操作成功');
                }else{
...
//        search: true,                       //是否显示表格搜索，此搜索是客户端搜索，不会进服务端，所以，个人感觉意义不大
        ...
        uniqueId: "job_number",                     //每一行的唯一标识，一般为主键列
...
// 	            }
        ...
        onEditableSave: function (field, row, oldValue, $el) {
            $('#userGrid').bootstrapTable("resetView");
            $.ajax({
                type: "post",
                url: "/user/edit",
                data: row,
